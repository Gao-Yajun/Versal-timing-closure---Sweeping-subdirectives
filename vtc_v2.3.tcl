...
namespace eval vtc {
  ...
  variable s1Usage { \
    ...
    Example:
      RTL-based project: 
      run_stage1 -parent synth_1 -ndw lmh
      Post-syntheis project:
      run_stage1 -ndw lmh -opt_design_en 0 
  }
  ...
  variable s2Args {
    ...
    {level.arg       high "Choose the value to high for subdirective"}
  ...
  variable s2Usage { \
    ...
    Syntax:
    run_stage2 [-ref_run] [-ndw] [-level] [minwhs] [is_pb_ready]
    ...
    Example:
      ...
      run_stage2 -ndw mh
      #Set minwhs to -0.1
      run_stage2 -ndw mh -minwhs -0.1
      #Set is_pb_ready to 0
      run_stage2 -ndw mh -minwhs -0.1 -is_pb_ready 0
  }
  ...
  proc get_s2_sd {placeDir {is_pb_ready 1}} {
    variable numSlr
    variable placeSubdir
    set s2Subdir [dict create]
    set i 0
    dict for {k subDict} $placeSubdir {
      set mykey [get_cletter $k]
      set p [dict get $subDict Phase]
      set v [dict get $subDict Value]
      set vLen [llength $v]
      puts "####INFO: $i -> k: $k -> Phase: $p -> Value: $v"
      set sd {}
      if {$vLen>0} {
        set upv [lrange $v 1 end]
        puts "####INFO: $upv"
        foreach iv $upv {
          set sd {}
          foreach ip $p {
            lappend sd ${ip}.${k}.${iv}
            if {[string equal $k ExtraTimingOpt]} {
              lappend sd ${ip}.ExtraTimingUpdate
            }
          }
          puts "####INFO: subdirective -> $sd"
          if {[string equal $k ExtraTimingOpt]} {
            switch -exact -- $iv {
              med {
                   if {[string equal $placeDir Default]} {
                     dict append s2Subdir ${mykey}_med $sd
                     puts "####INFO: place_design directive: $placeDir -> ${mykey}_med -> $sd"
                     continue
                   }
                  }
              high {
                    if {[string equal $placeDir Default]} {
                      dict append s2Subdir ${mykey}_high $sd
                      puts "####INFO: place_design directive: $placeDir -> ${mykey}_high -> $sd"
                      continue
                    } elseif {[string equal $placeDir Explore]} {
                      dict append s2Subdir ${mykey}_high $sd
                      puts "####INFO: place_design directive: $placeDir -> ${mykey}_high -> $sd"
                      continue
                    }
          ...
          } elseif {[string equal $k BalancedSLR]} {
            if {$numSlr>1 && $is_pb_ready==0} {
              dict append s2Subdir ${mykey}_${iv} $sd
              puts "####INFO: place_design directive: $placeDir -> ${mykey}_${iv} -> $sd"
              continue
            }
          } else {
            dict append s2Subdir ${mykey}_${iv} $sd
        ...
        dict append s2Subdir $mykey $sd
        puts "####INFO: place_design directive: $placeDir -> ${mykey} -> $sd"
      }
      incr i
    }
    set AltSpreadLogic_med {Floorplan.ForceSpreading.med GPlace.ForceSpreading.med GPlace.ReduceCongestion.med DPlace.ReducePinDensity.med}
    set AltSpreadLogic_high {Floorplan.ForceSpreading.high GPlace.ForceSpreading.high GPlace.ReduceCongestion.high DPlace.ReducePinDensity.high}
    set mykey [get_cletter AltSpreadLogic_M]
    dict append s2Subdir $mykey $AltSpreadLogic_med
    puts "####INFO: place_design directive: $placeDir -> ${mykey} -> $sd"
    set mykey [get_cletter AltSpreadLogic_H]
    dict append s2Subdir $mykey $AltSpreadLogic_high
    puts "####INFO: place_design directive: $placeDir -> ${mykey} -> $sd"
    set tb [xilinx::designutils::prettyTable]
    set header [list Name Value]
    $tb header $header
    dict for {skey svalue} $s2Subdir {
      $tb addrow [list $skey $svalue]
    }
    puts [string repeat # 100]
  ...
  # run_stage2 -ndw lmh
  # run_stage2 -ndw m
  # run_stage2 -level high 
  # run_stage2 -level med
  # run_stage2 -level both
  # run_stage2 -ndw mh -level high
  # run_stage2 -ndw lmh -level high -minwhs 0
  # run_stage2 -minwhs -0.2
  # For SSI devices
  # run_stage2 -is_pb_ready 0
  ...
  proc run_stage2 {args} {
    ...
    set s2Subdir [get_s2_sd $placeDir $isPbReady]
    # Set acceptable values for NET_DELAY_WEIGHT 
    if {$ndwLen>0} {
    ...
    set s2Runs {}
    set tb [xilinx::designutils::prettyTable]
    set header [list ID RunName place_design.subdirective]
    $tb header $header
    set id 1
    foreach i_ndw $ndwu {
      set tmpRun {}
      dict for {skey svalue} $s2Subdir {
        ...
        if {[string equal $level high] && [string match {*med} $skey]} { 
          puts "####INFO: $skey -> $level does not match"
          continue
        } elseif {[string equal $level med] && [string match {*high} $skey]} {
          puts "####INFO: $skey -> $level does not match"
          continue
        } else {
          puts "####INFO: $skey -> $level does match"
          ...
          puts "####INFO: $runName -> subdirective -> $svalue"
          puts "####INFO: copy run -> $runName -> done"
          lappend tmpRun [get_runs $runName]
          $tb addrow [list $id $runName $svalue]
          incr id
  ...
  proc get_untouched_subdir {usedSubdir availSubdir} {
    set res {}
  ...
  proc get_s3_sd {s2BestRun {level high}} {
    variable placeSubdir
    ...
    set unusedSubdir [get_untouched_subdir $s2SubdirName $availSubdir]
    puts "####INFO: unsed subdirectives in this run (stage 2) -> $unusedSubdir"
    switch -exact -- $level {
      med  { set val med  }
      high { set val high } 
      both { set val [list med high]}
      default {return -code error "####ERROR: Acceptable values of level -> med, high or both"}
    }
      ...
      if {[llength $v]>0} {
        ...
        foreach i_v $v {
          ...
          foreach i_p $p {
            ...
            if {[string equal $i_p ExtraTimingOpt]} {
  ...
  proc run_stage3 {args} {
    ...
    set s3Subdir [get_s3_sd $s2BestRun $level] 
    if {$ndwLen>0} {
    ...
    foreach i_ndw $ndwv {
      ...
      foreach i_s3Subdir $s3Subdir {
        ...
        if {[lsearch $i_s3Subdir {*BalancedSLR*}]!=-1} {
          if {$numSlr==1} {
            puts "####INFO: $i_s3Subdir -> #N SLR -> $numSlr -> Break out of this loop"
            continue
          } elseif {$numSlr>1} {
            if {$isPbReady==1} {
              puts "####INFO: $i_s3Subdir -> #N SLR -> $numSlr -> Pblock ready -> Break out of this loop"
              continue
            } else {
              puts "####INFO: $i_s3Subdir -> #N SLR -> $numSlr -> Pblock not ready" 
            }
          }
        }
        ...
        puts "####INFO: $runName settings done"
        $tb addrow [list $id $runName $i_s3Subdir]
